...
## 1. Purpose & Vision
The **Atomic Agent Framework** is a modular, verb-based AI orchestration system designed for a 2026-standard SDLC. Unlike "monolithic" AI prompts, this system breaks every phase of software development into **Atomic Skills**. 
...
3.  **Path Inference:** Agents must never hardcode file paths. They should infer paths from the `{{args}}` passed to them or from standard project structures (e.g., `./docs/ARCH.md`).
...
All agents must follow this structure in `~/.gemini/commands/atomic/`:
